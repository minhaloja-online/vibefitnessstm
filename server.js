...
app.post('/api/criar-preferencia', async (req, res) => {
  try {
    ...
    for (const item of pedido.itens) {
      ...
      // Confere o estoque da combinação exata antes de gerar a cobrança. Sem isso,
      // duas clientes podem pagar pela última peça ao mesmo tempo.
      ...
      if (disponivel < qtd) {
        const nomeCompleto = `${prod.nome}${variacaoTxt ? ' (' + variacaoTxt + ')' : ''}`;
        return res.status(409).json({
          erro: disponivel > 0
            ? `Restam apenas ${disponivel} de "${nomeCompleto}". Ajuste a quantidade no carrinho.`
            : `"${nomeCompleto}" ficou sem estoque. Remova o item do carrinho para continuar.`
        });
      }
    ...
    if (Math.abs(total - Number(pedido.total || 0)) > 0.01) {
      ...
      await refPedido.update({
        total,
        itens: itens.map(i => ({
          id: i.id, nome: i.title, preco: i.unit_price, qtd: i.quantity,
...
app.post('/api/webhook', async (req, res) => {
  ...
  try {
    ...
    await db.runTransaction(async tx => {
      ...
      // Se o produto tem grade de variações (cor/tamanho/numeração), a baixa é
      // feita na combinação exata que a cliente comprou, e o total é recalculado.
      (pedido.itens || []).forEach((item, i) => {
        ...
        if (Array.isArray(prod.variacoes) && prod.variacoes.length) {
          ...
          tx.update(refsProdutos[i], { estoque: Math.max(atual - qtd, 0) });
        }
      });
...
app.get('/api/status', (req, res) => {
  res.json({
    ...
    // Precisa ser EXATAMENTE o mesmo projectId que está no firebaseConfig do site.
